#!/usr/bin/env bash
# Import full v128 curriculum (16 chapters × 8 tasks = 128) into PostgreSQL.
# Run on the server after deploy, from repo root:
#   chmod +x deploy/scripts/import-curriculum-128.sh
#   ./deploy/scripts/import-curriculum-128.sh
#
# Chapters 1–5 only: ./deploy/scripts/import-chapters-1-5.sh
#
# Optional:
...
echo "==> Importing full v128 curriculum (128 tasks) into production database"
echo "    ENV_FILE=$ENV_FILE"
if [[ "${#IMPORT_ARGS[@]}" -gt 0 ]]; then
  echo "    extra args: ${IMPORT_ARGS[*]}"
fi

run_import import_algo_basics_ch1.py "Chapter 1 — algo basics (1–8)"
...
run_import import_inheritance_ch16.py "Chapter 16 — inheritance capstone (121–128)"

echo ""
echo "Done — 128 tasks imported. Restart API if it was already running:"
