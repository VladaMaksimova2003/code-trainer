...
def test_sequence_mismatch_is_student_safe_and_debug_keeps_expected_sequence():
    ...
    mismatch = next(
        err for err in result["errors"] if err["type"] == "FLOW_SEQUENCE_MISMATCH"
    )
...
def test_text_mismatch_is_student_safe_and_debug_keeps_patterns():
    ...
    mismatch = next(
        err for err in result["errors"] if err["type"] == "FLOW_TEXT_MISMATCH"
    )
...
def test_construction_missing_is_student_safe_and_debug_keeps_details():
    ...
    missing = next(
        err for err in result["errors"] if err["type"] == "FLOW_CONSTRUCTION_MISSING"
    )
...
def test_loop_back_edge_message_is_student_safe():
    ...
    loop_err = next(
        err for err in result["errors"] if err["type"] == "FLOW_LOOP_BACK_EDGE"
    )
