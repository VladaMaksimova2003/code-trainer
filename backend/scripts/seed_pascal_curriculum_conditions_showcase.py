#!/usr/bin/env python3
"""
Seed Pascal curriculum v2 showcase tasks for LC `conditions`.

Run:
  cd backend
  poetry run python scripts/seed_pascal_curriculum_conditions_showcase.py

Optional:
  poetry run python scripts/seed_pascal_curriculum_conditions_showcase.py --teacher-email admin@test.com
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed Pascal conditions curriculum showcase tasks")
    parser.add_argument(
        "--teacher-email",
    ...
    try:
        ...
        report = seed_pascal_conditions_showcase(session, teacher_id=teacher_id)
        session.commit()
