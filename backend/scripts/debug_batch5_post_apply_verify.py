"""Post-apply verification for debug import Batch 5 (task_068, task_076)."""
...
def main() -> None:
    ...
    for pid in PATTERNS:
        ...
        report["patterns"][pid] = {
            ...
            "public_payload": {
                ...
                "fixed_python_head": "\n".join(str(api_ce.get("python") or "").splitlines()[:4]),
                "buggy_python_head": "\n".join(str(api_ce.get("buggy_python") or "").splitlines()[:4]),
    ...
    cur.close()
    conn.close()
    out = BACKEND.parent / "docs" / "debug_import_batch5_итог.json"
