#!/usr/bin/env python3
"""Attach C++ language tracks to Universal Core (all stages or one MVP stage).

Run full course (237 tasks, 26 chapters):
  poetry run python scripts/seed_cpp_course_v311.py --stage all

Requires Pascal v3.1.1 showcase in DB for mirror attach rows (~118 slots).
Seed Pascal first if attach errors mention missing unified tasks:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed / attach C++ Course v1 tracks")
    parser.add_argument(
        "--stage",
        default="all",
