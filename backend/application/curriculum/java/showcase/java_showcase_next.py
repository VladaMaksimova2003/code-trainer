"""Next-task selection for Java showcase collections."""
...
from application.curriculum.java.showcase.java_v311_registry import v311_collection_by_key, v311_showcase_group
from application.curriculum.java.showcase.java_v311_showcase_all_specs import all_java_v311_showcase_specs
...
def order_collection_showcase_tasks(
    ...
    rows = list_java_tasks_for_collection(session, chapter_key)
...
def build_java_v311_showcase_collection_next(
    session: Session,
    chapter_key: str,
    user_id: int | None,
) -> dict[str, Any]:
    col = v311_collection_by_key(chapter_key)
    if col is None:
        raise ValueError(f"Unknown Java showcase chapter: {chapter_key}")

    specs = all_java_v311_showcase_specs().get(chapter_key, ())
