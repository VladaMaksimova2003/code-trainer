"""Build showcase specs for Java Course v1."""
...
def all_java_v311_showcase_specs() -> dict[str, tuple[dict, ...]]:
    grouped = records_by_chapter()
    return {key: tuple(record_to_spec_dict(r) for r in grouped[key]) for key in grouped}


@lru_cache(maxsize=None)
def java_v311_showcase_specs_for_collection(chapter_key: str) -> tuple[dict, ...]:
...
def specs_for_chapters(chapters: tuple[str, ...]) -> dict[str, tuple[dict, ...]]:
    all_specs = all_java_v311_showcase_specs()
