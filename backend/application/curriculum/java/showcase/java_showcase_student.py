"""Student-facing Java showcase view."""
...
from application.curriculum.java.showcase.java_showcase_next import order_collection_showcase_tasks, _row_from_task
...
def build_java_showcase_student_view(
    session: Session,
    chapter_key: str,
    *,
    ...
    if col is None:
        raise ValueError(f"Unknown Java chapter: {chapter_key}")

    batch_token = activate_batch_progress_cache(session, user_id)
    try:
        return _build_java_showcase_student_body(
            session,
            chapter_key,
            col=col,
            user_id=user_id,
...
def _build_java_showcase_student_body(
    session: Session,
    chapter_key: str,
    *,
    col,
