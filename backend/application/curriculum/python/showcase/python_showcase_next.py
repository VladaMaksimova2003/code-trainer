"""Next-task selection for Python showcase collections."""
...
from application.curriculum.python.showcase.python_v311_registry import v311_collection_by_key, v311_title_prefix
...
def order_collection_showcase_tasks(
    ...
    curriculum_version = "1.0" if specs and _is_v311_spec(specs[0]) else "2"
...
def build_python_showcase_collection_next(
    session: Session,
    chapter_key: str,
    user_id: int | None,
) -> dict[str, Any]:
...
def build_python_v311_showcase_collection_next(
    session: Session,
    chapter_key: str,
    user_id: int | None,
) -> dict[str, Any]:
...
def build_pascal_conditions_showcase_next(session: Session, user_id: int | None) -> dict[str, Any]:
    return build_python_showcase_collection_next(session, "conditions", user_id)


def build_pascal_loops_showcase_next(session: Session, user_id: int | None) -> dict[str, Any]:
    return build_python_showcase_collection_next(session, "loops", user_id)
