"""Seed Pascal curriculum loops showcase tasks + curriculum links."""
...
def _showcase_meta(spec: LoopsShowcaseTaskSpec) -> dict[str, str]:
    return {
...
def _attach_showcase_meta(row: TaskModel, spec: LoopsShowcaseTaskSpec) -> None:
    examples = dict(row.code_examples or {})
    examples["curriculum_showcase"] = _showcase_meta(spec)
    row.code_examples = examples


def _create_translation_python_to_pascal(
    session: Session,
    *,
    spec: LoopsShowcaseTaskSpec,
    teacher_id: int | None,
) -> int:
    extra = spec.extra or {}
...
_BUILDERS = {
    "translation_python_to_pascal": _create_translation_python_to_pascal,
...
def seed_pascal_loops_showcase(
    session: Session,
    *,
