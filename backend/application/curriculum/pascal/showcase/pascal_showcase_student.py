"""Student-facing Pascal showcase view (any collection)."""
...
_V311_CURRICULUM_VERSION = "3.1.1"
...
def _is_v311_col(col) -> bool:
    return isinstance(col, PascalV311Collection)
...
def build_pascal_showcase_student_view(
    session: Session,
    chapter_key: str,
    *,
    ...
    return run_with_showcase_task_index(
        session,
        lambda: _build_pascal_showcase_student_view_impl(
            session,
            chapter_key,
            user_id=user_id,
        ),
...
def _build_pascal_showcase_student_view_impl(
    session: Session,
    chapter_key: str,
    *,
    user_id: int | None = None,
