"""C# Course showcase collections — 192 tasks, 16 chapters (algorithm-syntax, v192-B)."""
...
def _col(chapter) -> CsharpV311Collection:
    return CsharpV311Collection(
        chapter_key=chapter.chapter_key,
        collection_id=f"csharp_{chapter.chapter_key}_v311",
        title_ru=chapter.title_ru,
        description_ru=chapter.description_ru,
        route_path=f"/learn/csharp/{chapter.route_suffix}",
        showcase_group=f"csharp_curriculum_v311_{chapter.chapter_key}",
...
CSHARP_V311_SHOWCASE_COLLECTIONS: tuple[CsharpV311Collection, ...] = tuple(
...
def v311_collection_by_key(chapter_key: str) -> CsharpV311Collection | None:
    for item in CSHARP_V311_SHOWCASE_COLLECTIONS:
...
def v311_showcase_group(chapter_key: str) -> str:
    col = v311_collection_by_key(chapter_key)
    return col.showcase_group if col else f"csharp_curriculum_v311_{chapter_key}"
