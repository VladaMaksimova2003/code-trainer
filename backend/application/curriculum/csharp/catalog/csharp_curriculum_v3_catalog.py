...
from application.curriculum.csharp.catalog.csharp_v311_builder_mapping import (
    assignment_for_task_format,
    builder_for_task_format,
)
from application.curriculum.csharp.catalog.csharp_v311_content import build_task_extra
...
from csharp_v31_tasks import V31_CHAPTER_TITLES, V31_TASKS  # noqa: E402

V311_TOTAL_TASKS = active_target_task_count()

LANG_PATTERN = re.compile(r"pascal|python|java|c\+\+|cpp", re.I)
...
def _is_algo_syntax_slot(slot_id: str) -> bool:
    return bool(re.fullmatch(r"cs_\d{3}", str(slot_id or "").strip()))
...
def record_to_spec_dict(rec: V311TaskRecord) -> dict:
    ...
    extra["expected_concept_ids"] = expected_concept_ids_for_row(
        rec.slot_id, rec.chapter_key, _features=rec.csharp_features
    )
    col = v311_collection_by_key(rec.chapter_key)
