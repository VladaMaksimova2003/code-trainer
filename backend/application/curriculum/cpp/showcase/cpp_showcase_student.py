...
from application.curriculum.cpp.showcase.cpp_showcase_next import order_collection_showcase_tasks
...
def build_cpp_showcase_student_view(
    session: Session,
    chapter_key: str,
    *,
    ...
    return run_with_showcase_task_index(
        session,
        lambda: _build_cpp_showcase_student_view_impl(
            session,
            chapter_key,
            user_id=user_id,
        ),
...
def _build_cpp_showcase_student_view_impl(
    session: Session,
    chapter_key: str,
    *,
    user_id: int | None = None,
    ...
    try:
        return _build_cpp_showcase_student_body(
            session,
            chapter_key,
            col=col,
            user_id=user_id,
...
def _build_cpp_showcase_student_body(
    session: Session,
    chapter_key: str,
    *,
    col,
    ...
    specs = all_cpp_v311_showcase_specs().get(chapter_key, ())
    ...
    if task_ids:
        for task in session.scalars(select(TaskModel).where(TaskModel.id.in_(task_ids))).all():
            ...
            from application.curriculum.cpp.showcase.cpp_showcase_next import _row_from_task
