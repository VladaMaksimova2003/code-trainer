"""C++ Course showcase collections — 128 tasks, 16 chapters (algorithm-syntax)."""
...
def _col(chapter) -> CppV311Collection:
    return CppV311Collection(
        chapter_key=chapter.chapter_key,
        collection_id=f"cpp_{chapter.chapter_key}_v311",
        title_ru=chapter.title_ru,
        description_ru=chapter.description_ru,
        route_path=f"/learn/cpp/{chapter.route_suffix}",
        showcase_group=f"cpp_curriculum_v311_{chapter.chapter_key}",
...
CPP_V311_SHOWCASE_COLLECTIONS: tuple[CppV311Collection, ...] = tuple(
...
def v311_collection_by_key(chapter_key: str) -> CppV311Collection | None:
    for item in CPP_V311_SHOWCASE_COLLECTIONS:
...
def v311_showcase_group(chapter_key: str) -> str:
    col = v311_collection_by_key(chapter_key)
    return col.showcase_group if col else f"cpp_curriculum_v311_{chapter_key}"
