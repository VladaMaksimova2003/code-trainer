"""Construction tags → concept ids from concepts.yml."""
from __future__ import annotations

from domain.learning.skill_groups import CONCEPT_IDS, CONSTRUCTION_TO_CONCEPTS
...
def normalize_construction_tags(tags: list[str]) -> list[str]:
    """Task tags → unique concept ids (stable order)."""
    seen: set[str] = set()
    ordered: list[str] = []
    for raw in tags:
...
def _concepts_for_tag(tag: str) -> list[str]:
    ...
    return [
        concept_id
        for concept_id in CONSTRUCTION_TO_CONCEPTS.get(normalized, ())
