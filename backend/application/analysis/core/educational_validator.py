...
from application.analysis.educational_vocabulary import (
...
from infrastructure.analysis.concept_ast_detector import (
    concept_label,
    detect_concepts,
)
...
class EducationalValidator:
    ...
    def missing_structure_labels(
        ...
        if not missing:
            return []
        return [
            f"Отсутствует конструкция: {concept_label(item)}"
            for item in missing
        ]
