...
from application.execution.services.flow_student_messages import student_flow_error_text
...
class FlowValidationService:
    ...
    def validate_with_details(
        ...
        required_sequence = [
            ...
            for item in (flow_spec.get("required_sequence") or [])
            if str(item).strip()
        ]
        ...
        if not normalized_nodes:
            return self._structural_result(
                ...
                debug={
                    ...
                    "reachable_types": [],
                    "detected_sequence": "",
                    "expected_sequence": format_sequence_labels(required_sequence)
                    ...
                    errors.append(
                        {
                            ...
                            "text": (
                                f"Блок #{index} имеет неподдерживаемый тип: "
                                f"«{label_block_type(node['type'])}»."
                            ),
                        }
                    )
        ...
        if nodes and edges is not None:
            graph_errors, reachable_sequence, adjacency, visited_ids = (
                self._validate_graph(normalized_nodes, edges)
            )
        ...
        if required_sequence:
            if flow_spec.get("allow_extra_nodes") is False:
                if reachable_types != required_sequence:
                    errors.append(
                        {
            ...
            elif not self._contains_subsequence(
                reachable_types, required_sequence
            ):
                errors.append(
                    {
                        "type": "FLOW_SEQUENCE_MISMATCH",
        ...
        if text_checks:
            text_errors, text_check_failures = self._validate_required_text_checks(
                reachable_sequence, text_checks
            )
            errors.extend(text_errors)

        if nodes and edges is not None and flow_spec.get("require_loop_back_edge"):
            errors.extend(
                self._validate_loop_back_edges(
                    nodes=reachable_sequence,
                    adjacency=adjacency,
                    visited_ids=visited_ids,
                )
    ...
    def _build_debug_info(
        *,
        ...
        detected_labels = [label_block_type(item) for item in reachable_types]
        debug: dict[str, Any] = {
            ...
            "disconnected_node_count": max(
                0, len(normalized_nodes) - len(visited_ids)
            ),
    ...
    def _validate_graph(
        ...
        if disconnected:
            labels = ", ".join(
                label_block_type(node["type"]) for node in disconnected
            )
            errors.append(
                {
        ...
        reachable_nodes = [node for node in nodes if str(node["id"]) in visited]
    ...
    def _validate_graph_topology(
        ...
        *,
        ...
        errors: list[dict[str, str]] = []
        end_reachable = any(
            node["type"] == "end" and str(node["id"]) in visited_ids for node in nodes
        )
                ...
                errors.append(
                    {
                        ...
                        "text": (
                            "У блока «Условие» должно быть две ветви "
                            "(да и нет)."
                        ),
                    }
                )
    ...
    def _validate_required_text_checks(
        ...
        for raw in text_checks:
            ...
            patterns = [
                str(item).strip().lower()
                for item in (
                    raw.get("contains_any") or raw.get("text_patterns") or []
                )
                if str(item).strip()
            ]
    ...
    def _has_loop_back_edge(
        ...
        queue = deque(
            ...
            if target in visited_ids and target != loop_id
        )
        while queue:
            current = queue.popleft()
        ...
        return any(
            loop_id in adjacency.get(source_id, [])
            for source_id in descendants
        )
