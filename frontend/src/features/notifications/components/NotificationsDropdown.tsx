...
function notificationPreview(n: unknown) {
  if ((n.kind === "comment" || n.kind === "ticket_reply") && n.body) {
    return {
      primary: n.body,
      secondary: n.title,
    };
...
export default function NotificationsDropdown() {
  ...
  const refreshCount = useCallback(() => {
    getUnreadCount().then((r: unknown) => setUnread(r.count || 0)).catch(() => {});
  }, []);
  ...
  const openItem = async (n: unknown) => {
    if (!n.read) {
      await markRead(n.id);
      setItems((prev: unknown) => prev.map((x: unknown) => (x.id === n.id ? { ...x, read: true } : x)));
      setUnread((c: unknown) => Math.max(0, c - 1));
    }
    setOpen(false);
  ...
  const onMarkAll = async () => {
    await markAllRead();
    setItems((prev: unknown) => prev.map((x: unknown) => ({ ...x, read: true })));
    setUnread(0);
  };
            ...
            {items.map((n: unknown) => {
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-[13px] font-medium text-ink line-clamp-2">
                    {preview.primary}
                  </span>
                  {preview.secondary && (
