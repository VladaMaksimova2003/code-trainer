...
interface SubmissionCommentsTabProps {
  ...
  currentTeacherId?: number | string | null
...
export default function SubmissionCommentsTab({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(data) ? (data as SubmissionComment[]) : []
      setComments(list)
  ...
  const handleCreated = (item: unknown) => {
    const comment = item as SubmissionComment
    setComments((prev) => {
  ...
  const handleUpdated = (item: unknown) => {
    const comment = item as SubmissionComment
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
        ...
        {comments.map((c) => {
          const own =
            isTeacherReview &&
            currentTeacherId != null &&
            String(c.teacher_id) === String(currentTeacherId)
